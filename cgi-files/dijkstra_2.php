...
class Dijkstra {
	...
	function printMap(&$map) {
		$placeholder = ' %' . strlen($this -> infiniteDistance) .'f';
		$foo = '';
...
$dijkstra = new Dijkstra($ourMap, I,$matrixWidth);
  ...
  //write_SimFile($lengthD, "theDFile ",0);
  // write_SimFile($dijkstra->printMap($ourMap), "theDFile", 0);
   
   // $dijkstra->findShortestPath(0,13); // to find only path from field 0 to field 13...
   $dijkstra->findShortestPath(0,$matrixWidth-1); 
 
  //write_SimFile("\n", "theDFile", 0);
 //write_SimFile($lengthD, "theDFile", 0);
 //write_SimFile("\n", "theDFile", 0);
 write_SimFile($lengthD, "theDFile", 1);
  write_SimFile("\n", "theDFile", 0);
 write_SimFile($dijkstra->printMap($ourMap), "theDFile", 0);
 write_SimFile("\n", "theDFile", 0);
 $results = $dijkstra -> getResults2($matrixWidth-1);
 for($i=0; $i< count($results[$matrixWidth-1]); $i++)
 {
 write_SimFile($results[$matrixWidth-1][$i] . " ", "theDFile",0);
 }
 //write_SimFile($result[$matrixWidth-1][$i] . " ", "theDFile",0);
...
// Display the results
echo array2js($results[$matrixWidth-1]);
?>
